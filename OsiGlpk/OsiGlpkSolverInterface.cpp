...
//
//    As of version 4.7, GLPK problems can be of class LP or GLP_MIP.
...
//
//    In (much) earlier versions of GLPK, if an GLP_MIP problem was
// changed back into a LP problem, then the MIP data was lost,
// including which columns are integer.  However, GLP_MIP problems
...
// all OSI problems are set to type GLP_MIP.  The only trick is
...
inline void checkGLPKerror (int err, std::string glpkfuncname, std::string osimethod) {
    if (err != 0) {
        ...
        std::cout << "ERROR: " << s;
        std::cout << " (" << osimethod << " in OsiGlpkSolverInterface)" << std::endl;
...
/*
  ...
  at least) will have solve_mip(lp, GLP_ON), a branch-and-cut solver. The presence of cut
...
void OGSI::branchAndBound ()
{
    ...
    */
    ...
    /*
      ...
      solve_mip(lp, GLP_ON) does not require an optimal LP solution as a starting point, so
      we can call it directly.

      solve_mip(lp, GLP_OFF) needs an initial optimal solution to the relaxation.
    */
    ...
    if (glp_get_num_int(model)) {
        ...
        /*
          ...
          Various other codes are returned by solve_mip(lp, GLP_ON) (solve_mip(lp, GLP_OFF) returns a
...
/*
  ...
  them into the GLP object. The trouble comes when loading a new problem. If
  the GLP object is already loaded with a problem, the easiest approach is to
  simply delete it and start fresh. But then we lose all the parameters that
  are held in the GLP object. By keeping a copy, we have a way to recover.
...
bool OGSI::setDblParam (OsiDblParam key, double value)
{
    ...
    switch (key) {
    case OsiDualObjectiveLimit: // as of 4.7, GLPK only uses this if it does dual simplex
    {
    ...
    case OsiPrimalObjectiveLimit: // as of 4.7, GLPK only uses this if it does dual simplex
    {
...
bool OGSI::setStrParam (OsiStrParam key, const std::string &value) {
    bool retval = false;
...
bool OGSI::setWarmStart (const CoinWarmStart* warmstart)
{
    ...
    */
    ...
    if (numcols != getNumCols() || numrows != getNumRows()) return false;
    ...
    */
    ...
    for (int j = 0; j < numcols; j++) {
        ...
        glp_set_col_stat(lp_, j+1, stati);
    }
    ...
    */
    {
        ...
        if (x) {
	        ...
	        std::cout << "OGSI(" << std::hex << this << std::dec;
		std::cout << ")::setColLower: new bound ";
		std::cout << lbj << " exceeds current value " << x[j];
		std::cout << " by " << (lbj-x[j]) << "." << std::endl;
            }
        }
    }
# endif
    ...
    */
    {
        ...
        if (x) {
	        ...
	        std::cout << "OGSI(" << std::hex << this << std::dec;
		std::cout << ")::setColUpper: new bound ";
		std::cout << ubj << " exceeds current value " << x[j];
		std::cout << " by " << (x[j]-ubj) << "." << std::endl;
            }
        }
    }
# endif
...
void OGSI::setColSetBounds(const int* indexFirst, const int* indexLast, const double* boundList) {
...
void OGSI::setRowType(int elementIndex, char sense, double rightHandSide, double range) {
    // Could be in OsiSolverInterfaceImpl.
    double lower = 0.0;
    double upper = 0.0;
...
void OGSI::setRowSetBounds(const int* indexFirst, const int* indexLast, const double* boundList) {
...
void OGSI::setColSolution(const double * cs) {
    ...
    if (cs == 0) delete [] colsol_;
    else {
        int nc = getNumCols();

        if (colsol_ == 0) colsol_ = new double[nc];
...
void OGSI::setRowPrice(const double * rs) {
    ...
    if (rs == 0) delete [] rowsol_;
    else {
        int nr = getNumRows();

        if (rowsol_ == 0) rowsol_ = new double[nr];
                        ...
                        const double *rowlb_parm, const double *rowub_parm)
{
    ...
    /*
      ...
      from the existing glp_prob structure.
...
void OGSI::writeMps(const char * filename,
    ...
    glp_write_mps(getMutableModelPtr(), GLP_MPS_DECK, NULL, const_cast<char*>(fullname.c_str()));
#else
...
glp_prob * OGSI::getMutableModelPtr(void) const {
    return lp_;
}
...
# ifdef LPX_K_USECUTS
    intParam = find_cps(srclpx)->use_cuts;
    find_cps(lpx)->use_cuts = intParam;
# endif
...
//  this->scale_   = 3;
    ...
    */
    ...
    /*
      ...
      USECUTS appears in 4.9; the parameter value
      appears to be unused in this version. LPX_C_ALL appears in 4.10.
    */
# if GLP_MAJOR_VERSION >= 4 && GLP_MINOR_VERSION >=9
#   if GLP_MAJOR_VERSION >= 4 && GLP_MINOR_VERSION >=10
    find_cps(lp_)->use_cuts = 0xFF; // LPX_C_ALL;
#   else
    find_cps(lp_)->use_cuts = 0;
#   endif
# endif
}
